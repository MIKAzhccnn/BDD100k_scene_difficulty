...
def resolve_val_dirs(yaml_path: Path):
    ...
    # 推断 labels 目录（把 images 替换为 labels；如果不需要可忽略）
    s = str(val_dir)
    if "/images/" in s:
...
def read_final_maps_from_results_csv(results_csv: Path):
    """
    读取 Ultralytics run 目录下的 results.csv 最后一行，
    返回 mAP50 和 mAP50-95（兼容不同列名）。
    """
...
def main():
    ...
    ap.add_argument("--batch", type=int, default=8)   # 8GB 卡通常 6~8 没问题
    ...
    # 优先从 run 的 results.csv 读取最终 map；如失败则从返回对象兜底
