...
# ---------------- Paths ----------------
root = Path('~/Projects/moca/results').expanduser()       # same as the original script (results root for All model)
all_json = root / 'exp_log.json'                          # All results (json should contain weather/mAP50/mAP50_95/lat_p50_ms)
ft_csv   = root / 'combined_weather_results.csv'          # FT summary generated by our helper script
...
if not ft_csv.exists():
    raise FileNotFoundError(f"FT summary not found: {ft_csv}\nRun weather_eval_all.py first to generate this file.")

df_ft_raw = pd.read_csv(ft_csv)
# Keep only Per-weather FT; if duplicates, keep the entry with highest mAP50_95
...
f_m5095  = arr_from(df_ft, 'mAP50_95')
    f_lat50  = arr_from(df_ft, 'p50_ms')   # FT's p50 column name
